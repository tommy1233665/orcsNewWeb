...
class NewFlightRiskDetail extends React.Component {
  //初始化表单数据
  constructor(props) {
    super(props);
    var btns = [
  ...
  componentDidMount() {
    //请求返回数据的url
    let url = "relieveMeasure/queryAllNewFlightInformation";
    ...
    post({
      ...
      success: (data) => {
        ...
        this.setState({
          ...
          steps: [data.flyFlitRiskDetailList.length, data.cruiseFlitRiskDetailList.length, data.landFlitRiskDetailList.length],
        });
      }
    });
  }
  ...
  render() {
    const { btns, currentBtn, flitInfo = {}, dataSource, dataSource2, flyRmArptRiskList, landRmArptRiskList,
      flyFlitRiskDetailList, cruiseFlitRiskDetailList, landFlitRiskDetailList,
      riskValue, table, modalOkBtnLoading } = this.state;
    //嘴上放表结构，航班信息。
    const list = [
...
class RiskTable extends React.Component {
  ...
  render() {
    //    const { title, value, environmentValue, machineValue, persionValue, riskDetailList } = this.props;
    const { title, value, riskDetailList } = this.props;
    //  var arr = this.handleList(riskList).concat( this.handleDetailList(riskDetailList) );
    var arr = this.handleDetailList(riskDetailList);
...
class TextHtml extends React.Component {
  ...
  handleHtml(str) {
    var arr = [];
    //处理详情描述中的点击事件onclick（显示气象报文，MEL数据等）
    if (str) {
...
export default connect(
  mapStateToProps
)(withRouter(NewFlightRiskDetail));